...
class ClientForm(QDialog):
    def __init__(
        ...
        for col in self.columns_meta:
            ...
            # skip pk field in add mode (DB will assign it)
        ...
        if self.mode == "add":
            self.btn_update.setEnabled(False)
            self.btn_delete.setEnabled(False)
            # in add mode, pk should be empty/read-only
    ...
    def on_add(self):
        ...
        if missing:
            ...
            return
        try:
            clients_repo.insert_row(self.cfg, data, self.pk_col)
    ...
    def on_update(self):
        ...
        if pk_val is None:
            ...
            return
        try:
            clients_repo.update_row(self.cfg, self.pk_col, pk_val, data)
    ...
    def on_delete(self):
        ...
        if ok != QMessageBox.StandardButton.Yes:
            return
        try:
            clients_repo.delete_row(self.cfg, self.pk_col, pk_val)
