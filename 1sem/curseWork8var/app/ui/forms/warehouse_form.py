...
class WarehouseForm(QDialog):
    def __init__(self, cfg: Config, role: str, record_id: Optional[int] = None, parent=None):
        ...
        self.qty_spin.setRange(0, 10 ** 9)
        layout.addWidget(self.qty_spin)

        layout.addWidget(QLabel("Закупочная цена:"))
        ...
        # RBAC: operators cannot modify
        if self.role != "admin":
            self.btn_add.setEnabled(False)
            self.btn_edit.setEnabled(False)
            self.btn_delete.setEnabled(False)
            # make fields read-only for non-admins (view-only)
        ...
        if self.record_id is not None:
            ...
            self.btn_delete.setEnabled(False)
            if self.role != "admin":
                # nothing to edit for non-admins
                self.btn_add.setEnabled(False)
        ...
        try:
            with get_conn(self.cfg) as conn:
                with conn.cursor() as cur:
                    cur.execute("SELECT id, name, quantity, amount FROM warehouses WHERE id = %s", (self.record_id,))
                    row = cur.fetchone()
            if not row:
                QMessageBox.critical(self, "Ошибка", "Запись не найдена")
                self.reject()
                return
            _id, name, quantity, amount = row
            self.name_edit.setText(name)
            try:
                self.qty_spin.setValue(int(quantity) if quantity is not None else 0)
            except Exception:
    ...
    def _validate(self) -> bool:
        ...
        if self.amount_spin.value() <= 0:
            QMessageBox.warning(self, "Validation", "Закупочная цена должна быть больше 0")
            return False
        if self.qty_spin.value() < 0:
            QMessageBox.warning(self, "Validation", "Количество должно быть неотрицательным")
            return False
        return True
    ...
    def on_add(self) -> None:
        ...
        try:
            with get_conn(self.cfg) as conn:
                with conn.cursor() as cur:
                    cur.execute(
                        "INSERT INTO warehouses(name, quantity, amount) VALUES (%s, %s, %s) RETURNING id",
                        (
                            self.name_edit.text().strip(),
                            int(self.qty_spin.value()),
                            float(self.amount_spin.value()),
                        ),
                    )
    ...
    def on_edit(self) -> None:
        ...
        try:
            with get_conn(self.cfg) as conn:
                with conn.cursor() as cur:
                    cur.execute(
                        "UPDATE warehouses SET name=%s, quantity=%s, amount=%s WHERE id=%s",
                        (
                            self.name_edit.text().strip(),
                            int(self.qty_spin.value()),
                            float(self.amount_spin.value()),
    ...
    def on_delete(self) -> None:
        ...
        try:
            with get_conn(self.cfg) as conn:
                with conn.cursor() as cur:
                    cur.execute("DELETE FROM warehouses WHERE id=%s", (self.record_id,))
                conn.commit()
