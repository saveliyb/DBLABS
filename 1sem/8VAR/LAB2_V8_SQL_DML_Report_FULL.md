...
Ты дал точную схему — **все запросы ниже строго под неё**:

### warehouses (склад)
- `id` — PK
- `name` — наименование
- `quantity` — количество на складе
- `amount` — стоимость 1 единицы товара (unit price)

### sales (продажи)
- `id` — PK
- `amount` — стоимость 1 единицы товара (unit price на момент продажи)
- `quantity` — количество
- `sale_date` — дата продажи
- `warehouse_id` — FK → `warehouses.id`

### expense_items (статьи расхода)
- `id` — PK
- `name` — наименование
...
## Общие правила этого файла
- Перед **каждым** пунктом есть **SETUP**: создаём тестовые данные с крайними значениями, и ожидаем в выводе **именно их**.
- Все тестовые данные помечены префиксом `T2_` → безопасно чистить.
- В местах, где нужно по методичке, **отключаем триггеры** на `charges`:
  ```sql
